...
class TimetableEntrySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'room', 'date', 'semester', 'department', 'department_name',
            'academic_year', 'is_active'
        ]

    def validate_faculty(self, value):
        if value.role != 'FACULTY':
            raise serializers.ValidationError("Selected user is not a faculty member.")
        return value
    ...
    def validate(self, data):
        ...
        # Check if faculty has another class at the same time
        if faculty and time_slot and date:
            conflicting_entry = TimetableEntry.objects.filter(
                faculty=faculty, time_slot=time_slot, date=date, is_active=True
            ).exclude(pk=getattr(self.instance, 'pk', None))
            if conflicting_entry.exists():
                raise serializers.ValidationError("Faculty already has a class scheduled at this time.")

        # Check if room is occupied at the same time (if room is provided)
        if room and time_slot and date:
            conflicting_entry = TimetableEntry.objects.filter(
                time_slot=time_slot, room=room, date=date, is_active=True
            ).exclude(pk=getattr(self.instance, 'pk', None))
            if conflicting_entry.exists():
                raise serializers.ValidationError("Room is already occupied at this time.")
