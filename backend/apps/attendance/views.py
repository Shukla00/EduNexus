...
class AttendanceSessionSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if request and getattr(request.user, 'role', None) == 'FACULTY':
            scheduled = TimetableEntry.objects.filter(
                faculty=request.user,
                course=data.get('course'),
                date=data.get('date'),
                semester=data.get('semester'),
                time_slot__start_time=data.get('start_time'),
                time_slot__end_time=data.get('end_time'),
                is_active=True,
            ).exists()
            if not scheduled:
                raise serializers.ValidationError(
                    "Attendance can only be created for your scheduled timetable slot."
                )
        return data
...
class MarkAttendanceView(APIView):
    ...
    def post(self, request):
        ...
        if getattr(request.user, 'role', None) == 'FACULTY' and session.faculty_id != request.user.id:
            return Response(
