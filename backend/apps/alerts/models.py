...
class Alert(models.Model):
    ...
    subject = models.CharField(max_length=100, null=True, blank=True) # For lecture reminders
    ...
    # 🔥 ADD THESE ML FIELDS
    ml_prediction = models.CharField(max_length=10, null=True, blank=True)
    ml_confidence = models.FloatField(null=True, blank=True)
    ml_avg_score = models.FloatField(null=True, blank=True)
    ...
    def __str__(self):
        target = self.student.enrollment_number if self.student else (
            self.faculty.email if self.faculty else 'N/A'
        )
