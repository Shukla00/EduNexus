...
export default function FacultyDashboard() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setTimetable(tt.data.results || tt.data)
  ...
  const todayClasses = timetable
    .filter(t => t.date ? t.date === todayValue : t.time_slot_detail?.day === dayAbbr)
    .sort((a, b) => (a.time_slot_detail?.start_time || '').localeCompare(b.time_slot_detail?.start_time || ''))
          ...
          <div className="space-y-3">
            {todayClasses.map(c => (
              <div key={c.id} className="flex items-center gap-4 p-3 bg-surface-50 rounded-xl border border-surface-200">
                ...
                <a className="btn-secondary btn-sm" href={`/faculty/attendance?date=${c.date || todayValue}&entry=${c.id}`}>Mark</a>
              </div>
            ))}
          </div>
        )}
