...
import { timetableApi, studentsApi, marksApi, examSchedulesApi } from '../../services/api'
...
export default function EnterMarks() {
  ...
  useEffect(() => {
    ...
    const loadExamSchedules = async () => {
      ...
      try {
        ...
        const [examRes, examTypesRes] = await Promise.all([
        ...
        setSemesters(
          Array.from(uniqueSemesters).sort()
        )

        setExamTypes(
          examTypesRes.data.results ||
          examTypesRes.data ||
          []
        )
  ...
  // Auto-select exam type if only one exists for the selected course
  useEffect(() => {
