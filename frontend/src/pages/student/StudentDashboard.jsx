...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const [sRes, aRes, mRes, alRes, pRes] = await Promise.all([
          studentsApi.me(),
          attendanceApi.mySummary(),
          marksApi.myMarks(),
          alertsApi.list({ resolved: 'false' }),
          marksApi.studentPerformance(),
        ])
  ...
  const marksBarData = performance.map(p => ({
    exam: `${p.course} - ${p.exam}`,
    marks: p.marks,
  }))
  console.log(performance)
  console.log(marksBarData)
  ...
  const risk = riskConfig[student?.ai_risk_level] || riskConfig
